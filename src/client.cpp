...
Client::Client(unsigned int client_id, unsigned int client_ip, unsigned int client_port)
{
    ...
    this->last_receive_time = std::asctime(std::localtime(&curr_time));
}

Client::Client(unsigned int client_id, unsigned int client_ip, unsigned int client_port, char *last_receive_time) : client_id(client_id),
                                                                                                                    client_ip(client_ip),
